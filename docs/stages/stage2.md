...
- **Keyboard-controlled debug player, not a real agent.** `scripts/run_match.py` drives player 0 from arrow keys + spacebar (shoot toward the opponent's goal) purely so a human can visually sanity-check the physics kernel — acceleration, speed clamping, friction, kicking, bounds/goals — in real time. This is *not* the Stage 3 `Agent` interface and shouldn't be mistaken for one; every other player just sits still.
